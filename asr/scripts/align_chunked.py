"""Force-align by cropping audio to each subtitle line, then merging back."""
import json
import sys
import numpy as np
...
OUTPUT = Path(sys.argv[3]) if len(sys.argv) > 3 else \
    PROJECT_ROOT / "aligned" / f"{AUDIO.stem}.aligned_chunked.json"

ALIGNER_MODEL = "/storage/external/lejun/Qwen3-ForcedAligner-0.6B"
BUFFER = 0.5  # seconds of padding on each side of the crop
...
def main():
    ...
    # Build per-line character list from characterAnnotations
    ...
    for line in lines:
        ...
        try:
            ...
            print(f"[OK]  {lid}: '{text}' → {len(segs)} segs")

        except Exception as e:
            n_failed += 1
            print(f"[ERR] {lid}: '{text}' — {e}")
            # fallback: spread chars evenly across the line window
